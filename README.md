...
- **即时降权**：授权撤销、有效期到期、账号停用都在**每次请求时实时判定**。停用会清空该用户的会话；前端每 3 秒、以及窗口重新聚焦/标签页重新可见时立即做一次**会话校验**，旧页面上已显示的明文（坐标/影像/备注）在下一次校验即收起并从 DOM 与内存清除，无需刷新或重登。
- **查看与详情审计**：
  - 列表/详情进入、受限字段“显示”、“隐藏”、导出都按**真实权限**记录操作者/时间/对象/结果；
  - 直接打开 `/#site=<id>` 深链进详情同样写“查看”；未登录打开详情/揭示/隐藏记一条匿名“拒绝”；
  - **隐藏不是纯前端动作**：无权隐藏（或字段名非法）服务端返回 403/400 并记“拒绝”，不会被记成成功；
  - 定时会话校验走 `silent` 通道，不刷审计日志。
- **查看隔离**：
  - 列表/详情只下发公共字段；坐标有权才给真值，无权则给一个确定性的**伪装占位坐标**（虚线斜纹标记，真实坐标不出服务端）；
  - 影像、备注明文只能通过“显示”按钮（`/reveal`）或导出获取，两条通道都写审计；
  - 任何服务端拒绝（401/403）返回后，前端在同一次处理中立即重新拉取最新权限、清除内存明文/影像草稿/隐藏表单值，无权明文不留在当前页面。
...
Playwright + Chromium，共 18 个用例，覆盖：详情进入（点击/深链）与未登录访问的查看审计、有权/无权隐藏的成功与拒绝记录、越权查看/写入/管理接口、受限字段筛选拒绝、授权过期与撤销后**下一次会话校验即清屏清内存**（focus 事件立即触发）、停用踢下线、双人并发 409 冲突且不覆盖、导出文件按人脱敏与明文字节扫描、非法/越权写入失败不改数据不升版本、手机视口全流程、旧标记入口回归。
