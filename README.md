...
- **即时降权**：授权撤销、有效期到期、账号停用都在**每次请求时实时判定**。停用会清空该用户的会话；前端每 3 秒、以及窗口重新聚焦/标签页重新可见时调用内部通道 **`POST /api/session/check`** 做一次会话校验，旧页面上已显示的明文（坐标/影像/备注）在下一次校验即收起并从 DOM 与内存清除，无需刷新或重登。
- **审计不可由客户端关闭**：所有真实读取无条件写审计，**不存在任何客户端静默开关**——在详情 URL 上附加 `?silent=1` 标记会被服务端忽略并照常记录；在列表请求里附加 `silent` 等未知字段会按白名单直接 400 拒绝并记一条拒绝审计。
- **内部校验通道最小化**：`/api/session/check` 只回 `{id, version, permissions}`（外加当前用户），不含编号、类型、潜次、深度、坐标、影像、备注甚至“是否存在受限数据”等存在性信息，**不是一次业务读取，因此不写查看/列表审计**，是否审计完全由服务端按端点决定。前端只拿它判断权限变化；撤权时本地清屏，新获授权时才走一次有审计的正常读取。
- **查看与详情审计**：
  - 列表/详情进入、受限字段“显示”、“隐藏”、导出都按**真实权限**记录操作者/时间/对象/结果（列表对象记为“遗址列表”）；
...
Playwright + Chromium，共 21 个用例，覆盖：伪造 `silent` 标记仍照常审计（详情忽略、列表 400 拒绝）、内部校验端点只回权限位不含业务数据且审计零增量、详情进入（点击/深链）与未登录访问的查看审计、有权/无权隐藏的成功与拒绝记录、越权查看/写入/管理接口、受限字段筛选拒绝、授权过期与撤销后**下一次会话校验即清屏清内存**（focus 事件立即触发）、停用踢下线、双人并发 409 冲突且不覆盖、导出文件按人脱敏与明文字节扫描、非法/越权写入失败不改数据不升版本、手机视口全流程、旧标记入口回归。
